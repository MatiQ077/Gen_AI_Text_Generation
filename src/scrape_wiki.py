import json
import time
from pathlib import Path
from typing import Dict, List, Optional

import pandas as pd
import requests
from bs4 import BeautifulSoup
from tqdm import tqdm

BASE_URL = "https://en.wikivoyage.org/wiki"
OUTPUT_PATH = Path("data/raw/wikivoyage_pages.jsonl")
CITY_LIST_PATH = Path("data/city_list.csv")

SECTION_CATEGORY = {
    "Understand": "city_description",
    "Get in": "transportation",
    "Get around": "transportation",
    "See": "attractions",
    "Do": "activities",
    "Buy": "shopping",
    "Eat": "food",
    "Drink": "nightlife",
    "Sleep": "accommodation",
    "Stay safe": "safety",
    "Connect": "practical_info",
}

#Download full HTML of a page
def fetch_page(url: str, timeout: int = 20) -> Optional[str]:
    
    #Create a HTTP header, so the website knows what the scrapped data is being used for
    headers = {"User-Agent": "TravelAssistantStudentProject"}

    try:
        response = requests.get(url, headers=headers, timeout=timeout)
        response.raise_for_status()
        return response.text
    
    except requests.RequestException as error:
        print(f"[ERROR] Could not fetch {url}: {error}")
        return None

#Basic text cleaning   
def basic_clean(text: str) -> str:

    text = text.replace("\n", " ")
    text = " ".join(text.split())
    return text.strip()

#Extract text from HTML by sections
def extract_sections(html: str) -> Dict[str, str]:

    soup = BeautifulSoup(html, "lxml")

    content = soup.find("div", {"id": "mw-content-text"})
    if content is None:
        return {}

    sections = {}
    current_section = None
    collected_text = []

    for element in content.find_all(["h2", "h3", "p", "ul"], recursive=True):
        if element.name in ["h2", "h3"]:            
            if current_section and collected_text:
                section_text = basic_clean(" ".join(collected_text))
                if section_text:
                    sections[current_section] = section_text

            headline = element.get_text(" ", strip=True)
            headline = headline.replace("[edit]", "").strip()

            current_section = None
            collected_text = []

            for known_section in SECTION_CATEGORY:
                if headline.lower().startswith(known_section.lower()):
                    current_section = known_section
                    break

        elif current_section:
            paragraph_text = element.get_text(" ", strip=True)
            paragraph_text = basic_clean(paragraph_text)

            if paragraph_text:
                collected_text.append(paragraph_text)
    
    if current_section and collected_text:
        section_text = basic_clean(" ".join(collected_text))
        if section_text:
            sections[current_section] = section_text

    return sections

#Scrape data for a single city from wikivoyage page
def scrape_city(country: str, city: str, slug: str) -> List[Dict]:
    
    #Slug is a unique and user-friendly part of URL that identifies a specific page, it replaces the usual numerical IDs
    url = f"{BASE_URL}/{slug}"
    html = fetch_page(url)

    if html is None:
        return []

    sections = extract_sections(html)
    records = []

    for section_name, section_text in sections.items():
        if len(section_text) < 50:
            continue

        record = {
            "source": "wikivoyage",
            "url": url,
            "country": country,
            "city": city,
            "section": section_name,
            "category": SECTION_CATEGORY.get(section_name, "other"),
            "text": section_text,
        }
        records.append(record)

    return records

#Append scrapped data to JSONL format, it is more convenient than regular JSON
#Each line is a valid, independent JSON object, easier to process the data if we have a big dataset
def append_jsonl(records: List[Dict], output_path: Path) -> None:
    
    output_path.parent.mkdir(parents=True, exist_ok=True)

    with output_path.open("a", encoding="utf-8") as file:
        for record in records:
            file.write(json.dumps(record, ensure_ascii=False) + "\n")

def main() -> None:
    cities_df = pd.read_csv(CITY_LIST_PATH)

    if OUTPUT_PATH.exists():
        OUTPUT_PATH.unlink()

    total_records = 0

    for _, row in tqdm(cities_df.iterrows(), total=len(cities_df)):
        country = row["country"]
        city = row["city"]
        slug = row["wikivoyage_slug"]

        records = scrape_city(country, city, slug)
        append_jsonl(records, OUTPUT_PATH)

        total_records += len(records)

        time.sleep(1)

    print(f"Done. Saved {total_records} records to {OUTPUT_PATH}")

if __name__ == "__main__":
    main()